...
public class CannonLogic {
	...
	void update ( TextureAtlas textureAtlas ) {
		for ( final @NotNull CannonBehavior entity : entities ) {
			...
			if ( input.isKeyPressed ( entity.getUserInputSpaceShipComponent ().getPrimaryCannons () ) && TimeUtils.timeSinceMillis (
					cannonComponent.getMillisecondsOfLastProjectile () ) > cannonComponent.getMillisecondsPerProjectile () ) {
				...
				try {
					ProjectileBehavior projectile = cannonComponent.getProjectile ().getDeclaredConstructor (
							PositionRotationComponent.class,
							VelocityComponent.class,
							int.class,
							TextureAtlas.class ).newInstance ( new PositionRotationComponent ( positionRotationComponent.getPosition ().cpy (),
																							   rotationDegrees ),
